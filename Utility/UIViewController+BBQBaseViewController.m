...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        swizzleMethod(class, @selector(didReceiveMemoryWarning), @selector(bbq_didReceiveMemoryWarning));
    });
}
...
- (void)bbq_didReceiveMemoryWarning {
    [self bbq_didReceiveMemoryWarning];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//       [MTA trackError:<#(NSString *)#>]
    });
}
