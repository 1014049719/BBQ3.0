...
+ (void)autoLogin {
    ...
    NSString *password = [[NSUserDefaults standardUserDefaults] valueForKey:[NSString stringWithFormat:@"password%@", [TheCurUser.member.uid stringValue]]];
...
#ifdef TARGET_PARENT
    if (TheCurUser && TheCurUser.baobaodata.count) return YES;
#else
